...
IVector2 Project(const FVector3& p, int screenW, int screenH) {
    ...
    const float camOffset = 4.f;   // pull the geometry forward of the camera plane

    float zCam = p.z + camOffset;
    if (zCam < 0.1f) zCam = 0.1f;

    float halfW = static_cast<float>(screenW) * 0.5f;
    float halfH = static_cast<float>(screenH) * 0.5f;

    int sx = static_cast<int>(halfW + (p.x * fov / zCam) * halfW);
    // Flip Y because screen Y grows downward.
    int sy = static_cast<int>(halfH - (p.y * fov / zCam) * halfH);
    return { sx, sy };
}
...
void RasterizerDemo::Run() {
    ...
    while (!WindowShouldClose()) {
        ...
        // Project to screen.
        std::array<IVector2, 3> screenPos;
        std::array<float, 3>     zPos;
        for (int i = 0; i < 3; ++i) {
            screenPos[i] = Project(world[i], _Width, _Height);
        ...
        if (mode == 1) {
            Rasterizer::Draw(RasterMode::DDA_LINE, screenPos[0], screenPos[1], screenPos[2], WHITE, &screen);
        } else if (mode == 2) {
            Rasterizer::Draw(RasterMode::BBOX_TRIANGLE, screenPos[0], screenPos[1], screenPos[2], SKYBLUE, &screen);
        } else {
            Gpu gpu;
            gpu.Mode = GpuDrawMode::COLOR;
            gpu.pointLightPosition = { 2.f, 2.f, -2.f };
            gpu.cameraPosition      = { 0.f, 0.f, -4.f };

            std::array<GpuVertex, 3> verts;
            for (int i = 0; i < 3; ++i) {
                verts[i].screenPosition = screenPos[i];
                verts[i].color      = baseColors[i];
                verts[i].zPosition      = zPos[i];
                verts[i].uv         = { 0.f, 0.f };
